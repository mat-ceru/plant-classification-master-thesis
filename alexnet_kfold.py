...
def get_subsets(base_set: torchvision.datasets.ImageFolder, splitting_ratio: int):
  ...
  return get_subset_indexes(base_set, s1_indexes, s2_indexes)

def get_subset_indexes(base_set: torchvision.datasets.ImageFolder, s1_indexes, s2_indexes):
...
train_and_val_dataset_labels = []
for _, l in train_and_val_dataset:
  train_and_val_dataset_labels.append(l)
...
sk_fold = StratifiedKFold(n_splits=K_FOLD)

with open(f"{OUTPUT_FILE_DIRECTORY}/{datetime.datetime.now()}-output.txt", "w") as f:
  for set_ in hyperparameters_sets:

    skf_val_accuracies = []; skf_val_losses = []; skf_val_true_positives = []
    skf_val_true_negatives = []; skf_val_false_positives = []; skf_val_false_negatives = []

    print(f'set: {set_}')
    
    for n_subset, (train_indexes, val_indexes) in enumerate(sk_fold.split(train_and_val_dataset, train_and_val_dataset_labels)):
      train_dataset, val_dataset = \
        get_subset_indexes(train_and_val_dataset, train_indexes, val_indexes)

      print(f'subset {n_subset} of set {set_}')
      ...
      skf_val_accuracies.append(val_accuracy)
      skf_val_losses.append(val_loss)
      skf_val_true_positives.append(val_true_positive)
      skf_val_true_negatives.append(val_true_negative) 
      skf_val_false_positives.append(val_false_positive) 
      skf_val_false_negatives.append(val_false_negative)

      f.flush()
        
    val_accuracies.append(skf_val_accuracies)
    val_losses.append(skf_val_losses)
    val_true_positives.append(skf_val_true_positives)
    val_true_negatives.append(skf_val_true_negatives) 
    val_false_positives.append(skf_val_false_positives) 
    val_false_negatives.append(skf_val_false_negatives)

    avg_val_accuracy = sum(skf_val_accuracies) / K_FOLD
    avg_val_loss = sum(skf_val_losses) / K_FOLD
    avg_val_true_positive = sum(skf_val_true_positives) / K_FOLD
    avg_val_true_negative = sum(skf_val_true_negatives) / K_FOLD
    avg_val_false_positive = sum(skf_val_false_positives) / K_FOLD
    avg_val_false_negative = sum(skf_val_false_negatives) / K_FOLD

    if avg_val_accuracy > best_accuracy:
      best_accuracy = avg_val_accuracy
      best_loss = avg_val_loss
      best_true_positives = avg_val_true_positive
      best_true_negatives = avg_val_true_negative
      best_false_positives = avg_val_false_positive
      best_false_negatives = avg_val_false_negative
